...
| IsTemplate | bit | 已废弃（由 MonitorEntity.IsTemplate 计算属性替代） |
...
索引：`HeartbeatAtUtc`（看门狗用）

## OutboxMessage — 事务发件箱

| 列 | 类型 | 说明 |
|----|------|------|
| Id | nvarchar(50) PK | Guid 字符串 |
...
|----|------|------|
| Id | Guid PK | |
| OutboxMessageId | nvarchar(50) | 关联 Outbox |
| ExecutionRecordId | Guid? | 关联执行记录 |
| Type | varchar(100) | TestFailure/ApiError/PerformanceIssue/... |
...
| PromptVersion | varchar(50) | Prompt 版本 |
| CreatedAt | datetime2 | |
...
| - | AddMonitorTemplateFields | IsTemplate + TemplateVariablesJson |
