...
public class MonitorRepository : IMonitorRepository
{
    ...
    public async Task<IEnumerable<MonitorEntity>> GetPendingTasksAsync()
    {
        try
        {
            _logger.LogDebug("Fetching pending monitors");

            var sql = @"SELECT Id, Name, Status, LastRunTime, IsEnabled, TargetType, TargetConfig
                        FROM Monitor
                        WHERE Status = @Pending AND IsEnabled = 1";

            var result = await _dbConnection.QueryAsync<MonitorDto>(sql, new { Pending = (int)MonitorStatus.Pending });
            ...
            _logger.LogDebug("Fetched {Count} pending monitors", entities.Count());
            return entities;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to fetch pending monitors");
            throw;
        }
    }
}